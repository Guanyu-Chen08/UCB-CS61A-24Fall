def divide(quotients, divisors):
    ...
    """
    return {keys: [multiples for multiples in divisors if multiples % keys == 0] for keys in quotients}
...
def closer_city(lat, lon, city_a, city_b):
    ...
    d_a = distance(city_target, city_a)
    d_b = distance(city_target, city_b)
    if d_a <= d_b:
