...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    assert player_score >= 0 and opponent_score >=0, "A score must be non-negative."
    "*** YOUR CODE HERE ***"
    a = player_score % 10
    b = opponent_score // 10
    if a == b:
...
def is_always_roll(strategy, goal=GOAL):
    ...
    "*** YOUR CODE HERE ***"
    ans = True
    strategy_num = strategy(0, 0)
    for i in range(goal + 1):
        for j in range(goal + 1):
            if strategy(i, j) != strategy_num:
                ans = False
    return ans
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    ans = 0
    for i in range(1, 11):
        temp_dice = make_averaged(roll_dice, times_called)
        temp_score = temp_dice(i, dice)
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    Suss Fuss rules."""
    # BEGIN PROBLEM 11
    result = score + boar_brawl(score, opponent_score)
...
def final_strategy(score, opponent_score, dice=six_sided, times_called = 1000):
    """Write a brief description of your final strategy.

    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    six_dice_result = make_averaged(dice, times_called)()
    if six_dice_result > boar_brawl(score, opponent_score):
        return 6
    return 0
    # END PROBLEM 12
